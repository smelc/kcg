...
creatureOptions = defaultOptions
  { fieldLabelModifier = impl }
  where impl "creatureName" = "name"
...
neutralOptions = defaultOptions
  { fieldLabelModifier = impl }
  where impl "neutralName" = "name"
        impl s = s
...
someFunc = do
  ...
  print decoding2
  print allDecoding
  where creature :: Creature = Creature Human "archer" 1 1 2 Nothing
